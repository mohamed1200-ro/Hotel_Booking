...
const HotelCard = ({ room, index }) => {
  return (
    <Link
      ...
      key={room?._id} // Keep if used in a list (e.g., map); otherwise, remove
    ...
    >
      <img
        src={room?.images?.[0] || ""}
        alt={room?.hotel?.name ? `${room.hotel.name} image` : "Hotel image"}
      />
      ...
      <div className="p-4 pt-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <img src={assets.starIconFilled} alt="Star rating icon" /> 4.5
          </div>
        </div>
