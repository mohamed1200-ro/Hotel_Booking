      ...
      <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
        ...
        <div className="flex items-center gap-1 mt-2">
          {Array(5)
            .fill(0)
            .map((_, index) => (
              <Star key={index} filled={roomsDummyData.rating > index} />
            ))}
          ...
          <div className="grid grid-cols-2 gap-4 lg:w-1/2 w-full">
            {room?.images.length > 1 &&
              room.images.map((image, index) => (
                <img
                  onClick={() => setRoomImage(image)}
                  key={index}
                  src={image}
                  alt="Room Image"
                  className={`w-full rounded-xl shadow-md object-cover
                        cursor-pointer ${roomImage === image && "outline-3 outline-orange-500"}`}
                />
              ))}
          </div>
        </div>
        ...
        <div className="flex flex-col items-start gap-4">
          <div className="flex gap-4">
            <img
              ...
              alt="Host"
              className="h-14 w-14 
          md:h-18 md:w-8/12 rounded-full"
            />
            <div>
          ...
          <button
            className="px-6 py-2.5 mt-4 rounded text-white bg-blue-600
